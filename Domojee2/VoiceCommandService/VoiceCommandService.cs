...
namespace Domojee.VoiceCommands
{
    ...
    public sealed class DomojeeVoiceCommandService : IBackgroundTask
    {
        ...
        public async void Run(IBackgroundTaskInstance taskInstance)
        {
            ...
            if (triggerDetails != null && triggerDetails.Name == "DomojeeVoiceCommandService")
            {
                try
                {
                    ...
                    // Depending on the operation (defined in Domojee:DomojeeCommands.xml)
                    // perform the appropriate command.
                    switch (voiceCommand.CommandName)
                    {
                        case "TemperaturePiece":
                            var destination = voiceCommand.Properties["commande"][0];
        ...
        private async Task SendCompletionMessageForDestination(string destination)
        {
            ...
            IEnumerable<JdObject> JeedomObject = RequestViewModel.ObjectList.Where(p => p.name == destination);
            
            var userMessage = new VoiceCommandUserMessage();
            var destinationsContentTiles = new List<VoiceCommandContentTile>();
            if (JeedomObject.Count() == 0)
            {
